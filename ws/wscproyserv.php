...
function inserta_os($id_local, $usuario){
	...
	*/
	...
	$query = "INSERT INTO os (id_os, id_estado, id_proyecto, id_local, id_direccion, clie_rut, os_fechacreacion, os_fechacotizacion, 
            os_fechaestimada, os_comentarios, os_numboleta, os_fechaboleta, os_descripcion, usuario, USR_ID, os_fechaestimacion, origen) 
			VALUES ( null,'".ESTADO_COTIZACION_DUMMY."',1,$id_local,1,".ID_CLIENTE_DUMMY.",now(),now(),now(),'',null,null,'".COMMENT_COTZ."','$usuario',0,adddate(now(),1),
				'".ESTADO_ORIGEN_PATIO."')";
...
function inserta_os_detalle($id_os,$price,$qty,$cod_prod1,$id_producto, $des_corta,$barcode){
	...
	//consulta ind_cat
	$my_barcode = $barcode + 0;

	$qry = "SELECT ind_decimal 
		FROM codbarra c 
		JOIN  unidmed u on c.unid_med = u.unid_med
		WHERE cod_barra = '$my_barcode'";
	wlog("inserta_os_detalle : MYSQL qry: $qry");
    $db_1 = tep_db_query($qry);
	if ( $res = tep_db_fetch_array( $db_1 ) ) {
		$ind_dec = $res['ind_decimal'];
		wlog("inserta_os_detalle : ind_dec = $ind_dec");
		}
	else {
		wlog ("inserta_os_detalle : No encuentra ind_dec: ".$qry);
		$ind_dec = 0;
		}
	...
	*/
	...
	$query = "INSERT INTO os_detalle (id_os_detalle,ot_id,id_origen,id_tipodespacho,id_os,osde_tipoprod,osde_subtipoprod,osde_instalacion,osde_precio,osde_cantidad,osde_descuento,osde_preciocosto,cod_sap,osde_especificacion,osde_descripcion,id_producto,cod_barra,usrnomaut,usrpassaut,observacion, ind_dec) VALUES(
            null,null,5,1,$id_os,'".TIPO_PRODUCTO_STOCK."','".TIPO_PRODUCTO_STOCK."',0,$price,$qty,0,0,'$cod_prod1','','$des_corta',$id_producto,'$my_barcode',null,null,null,$ind_dec)";
