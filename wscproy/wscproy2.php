...
Cliente : EASY
Autor : Rodrigo González Maldonado
BBR-Ingeniería
*********************************/
...
function Cambia_Estado($enc,&$msg_val){
	...
	$ot_id=Cambia_Estado_Ot( array_merge ($ARR, array('idDespacho'=>$idDespacho),array('id_estado'=>$id_estado),array('ot_id'=>$ot_id),array('usrcrea'=>$usrcrea),array('msgod'=>$msgod)),$msg_val);
	if (($ot_id==0)&&($msg_val!='')){
		return false;
	}
...
$server->configureWSDL('wscproy2', 'urn:wssaveco');
...
$server->register('Graba_OS',						// method name
    array('req' => 'xsd:string'),					// input parameters
    array('resp' => 'xsd:string'),					// output parameters
    'urn:wscproy2',									// namespace
    'urn:wscproy2#wssaveco',							// soapaction
    'rpc',											// style
    'encoded',										// use
    'Crea cotizacion en estado En trabajo'			// documentation
);
...
$server->register('Cons_SAP',						// method name
    ...
    'rpc',											// style
    'encoded',										// use
    'Consulta barras de un producto por SAP'		// documentation
);
