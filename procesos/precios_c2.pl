...
local $max_reg = 100000; 
	...
	if ($total_reg) {
		while ($count_reg <= $total_reg ) {
			...
			$qry = "INSERT INTO trp_precios (cod_local,cod_prod1,prec_valor,car_estadocarga,id_cadena,id_orden) 
					SELECT cod_local,cod_prod1,prec_valor,0, id_cadena ,id_orden
					FROM car_precios 
					WHERE id_carga = $idcarga
					ORDER BY prec_valor ASC
...
local $cod_prod1, $cod_local, $Pprec_valor,$prec_valor,$prec_costo, $stock ,$TPcod,$PRcod;
local $id_producto,$id_local;
    ...
    $qry = "select distinct TP.prec_valor 'Pprec_valor', p.id_producto, L.id_local, TP.cod_prod1 'TPcod', TP.cod_local,TP.id_orden
			from trp_precios TP
			join productos p on (p.cod_prod1 = TP.cod_prod1)
			join locales L on (L.cod_local = TP.cod_local)
			where TP.car_estadocarga = 0
			order by id_local,TP.prec_valor ASC";
	$sth = $dbh->prepare($qry);
    $rc = $sth->execute();
    ...
    while(($Pprec_valor,$id_producto,$id_local,$TPcod,$cod_local) = $sth->fetchrow_array) {
        ...
        if (exec_sql_non_select($qryupd)<=0) {
			...
			if ($DBI::err == "") {
				...
				$qryins = "	INSERT INTO precios (id_producto, cod_prod1, id_local, cod_local, prec_valor)
							VALUES ($id_producto,'$TPcod',  $id_local, '$cod_local', '$Pprec_valor')";
   ...
   $qryupe = "
		...
		SELECT distinct 'precios_c2.pl', 'ERROR', now(), 'procesa_UpdPrecios', concat('El producto ', cod_prod1, ' no existe en el catalogo. No se puede actualizar el precio ', prec_valor, ' en el local ', cod_local), 0
