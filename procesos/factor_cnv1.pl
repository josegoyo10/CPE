		...
		if (open (ARCH, "$Path_load/$array_element")) {
			...
			while ($linea=<ARCH>) {
				...
				if ( length($linea)>41 ){
					...
					push @a,  substr($linea, 18,3);
					push @a, (substr($linea, 21,18)+0);
					push @a, $j;
					&myprecios(@a);
					pop @a;
					pop @a;
...
sub myprecios(@a) {
   ...
   $qsql = "INSERT INTO car_codbarra (cod_prod1, unid_med, factor_conv, idcarga, id_cadena) 
   VALUES ('@a[0]', '@a[1]', '@a[2]', $idcarga, 3);";
...
sub main {
    ...
    $qry = "SELECT prefijo FROM archivos where tipo ='UMA' order by 1";
    $sth = $dbh->prepare($qry);
    $rc = $sth->execute();
