...
sub carga_arch_generico {
		...
		if ((substr($array_element, -4, 4) eq $Extension) || $Extension eq "") {
			if ($ncomp) {
				...
				for ($k=0; $k<=$#b; ++$k) {
					...
					if ( substr($array_element, 0, 17) . $Extension_vrf eq $b[$k] ) {
						#Se encontro el archivo de verificacion
						$okv = 1;
                                		for ($m=0; $m<=$#b; ++$m) {
							#busco si existe archivo original
                                        		if ( substr($array_element, 0, 17) . $Extension eq $b[$m] ) {
	...
	{
		wtlog("Entidad de carga :PRECIOS, archivo de carga :$array_element", "carga_arch_generico", 0);
		if (open (ARCH, "$Path_load/$array_element")) {
			...
			while ($linea=<ARCH>) {
				...
				if ( length($linea)>34 ){
					...
					push @a, substr($linea, 0,4);
					push @a, (substr($linea, 4,18)+0);
					push @a, (substr($linea, 22,10)+0);
					push @a, $j;
					&myprecios(@a);
					pop @a;
					pop @a;
		...
		unlink ("$Path_load/" . substr($array_element, 0, 17) . $Extension_vrf);
	}
...
sub myprecios(@a) {
	...
	$qsql = "INSERT INTO car_precios(cod_local, cod_prod1, prec_valor, id_carga, id_cadena,id_orden) 
			 VALUES					('@a[0]',	'@a[1]',   '@a[2]',	   $idcarga, 3,   '@a[3]');";
...
sub main {
	...
	my $Path_load		= $config->pc1_path_load;
    ...
    $qry = "SELECT prefijo FROM archivos where tipo ='PRE' order by 1";
    $sth = $dbh->prepare($qry);
    $rc = $sth->execute();
