<?php
$pag_ini = "../seleccionar_cliente/seleccionar_cliente.php";
...
$apellidos = $_POST['apellidos'];
	...
	$queryClie =  "SELECT clie_rut, clie_nombre, clie_paterno, clie_materno FROM clientes WHERE clie_tipo='p' AND ";
	
	if($_POST['cedula'] != '')	
	$queryClie =  $queryClie . " clie_rut = $cedula ";    
	
	if($_POST['nombres'] != '' && $_POST['cedula'] != '')
	$queryClie =  $queryClie . " AND UPPER(clie_nombre) LIKE UPPER('%$nombres%') ";
	
	if($_POST['nombres'] != '' && $_POST['cedula'] == '')
	$queryClie =  $queryClie . " UPPER(clie_nombre) LIKE UPPER('%$nombres%') ";
	
	if($_POST['apellidos'] != '' && ($_POST['cedula'] != '' || $_POST['nombres'] != ''))
	$queryClie =  $queryClie . " AND (UPPER(clie_paterno) LIKE UPPER('%$apellidos%') OR UPPER(clie_materno) LIKE UPPER('%$apellidos%')) ";
	
	if($_POST['apellidos'] != '' && ($_POST['cedula'] == '' && $_POST['nombres'] == ''))
	$queryClie =  $queryClie . " (UPPER(clie_paterno) LIKE UPPER('%$apellidos%') OR UPPER(clie_materno) LIKE UPPER('%$apellidos%')) ";
	...
	if($numero == 0)
	{
		...
		echo "alert('No existen clientes creados para la consulta solicitada')";
		echo "</script>";
	}
...
function colocar_valor()
{
	if(Nombre == '')
	{
	alert('Por favor seleccione un cliente y oprima Aceptar');
	}
	else{
	window.opener.viaje.clien.value = Nombre;
	window.opener.viaje.rut.value = Cedula;
	window.close();
	}
}
...
<body>
<p align="center" class="titulonormal"><strong>BUSQUEDA DE CLIENTE</strong></p>
<form name="forma" method="post" action="seleccionar_cliente.php" onSubmit="return validar();">
...
?>
  ...
  <tr id="<?php echo $ResulClie['clie_rut']; ?>" bgcolor="#E9E9E9" class="tabla2" onClick="concatenar(this.getElementsByTagName('td')[1].innerHTML + ' ' + this.getElementsByTagName('td')[2].innerHTML, this.getElementsByTagName('td')[0].innerHTML); cambiar_color_over(this); cambiar_color(id); " >
    <td><?php echo $ResulClie['clie_rut']; ?></td>
    <td id="<?php echo $ResulClie['clie_rut']; ?>"><?php echo $ResulClie['clie_nombre']; ?></td>
    <td ><?php echo $ResulClie['clie_paterno'] . " " . $ResulClie['clie_materno'];?></td>
  </tr>
