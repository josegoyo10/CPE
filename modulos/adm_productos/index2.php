...
function DisplayListado() {
    ...
    if ( $bus_1){
        ...
        if ($radiobutton1 == 1){
            $where_aux="JOIN codbarra cb ON cb.id_producto = p.id_producto AND cb.estadoactivo='C'
                        WHERE p.cod_prod1 ='".$bus_1."'";
        }
        else if ($radiobutton1 == 2){
            $where_aux="JOIN codbarra cb ON cb.id_producto = p.id_producto AND cb.estadoactivo='C' AND cb.cod_barra ='".$bus_1."'";
        }
    ...
    if ( $bus_2 ){
        ...
        if ($radiobutton2 == 1){ 
            $where_aux="JOIN prodxprov pxv ON pxv.cod_prod1 = p.cod_prod1
                        ...
                        JOIN codbarra cb ON cb.id_producto = p.id_producto AND cb.estadoactivo='C'
