<?
function cambia_estado($id_ot,$destino,$tipoDes,$id_estado,$esta_tipo,$accion,$ot_tipo) {
/* si la accion es cerrar pick del boton se igualan las cantidades de la ot*/
	if ($accion=="cp"){		
		
	$qryS=" select id_os_detalle,osde_cantidad from os_detalle where ot_id=".($id_ot+0);
	writelog("Cambia Estado 1: ". $qryS);
		if ( $rq = tep_db_query($qryS) ){
			while( $res = tep_db_fetch_array( $rq ) ) {
				if($destino != 'PN')
				{
				$qU="update os_detalle SET cant_pickeada=".$res['osde_cantidad']." where ot_id=".($id_ot+0)." and id_os_detalle=".$res['id_os_detalle'];
				tep_db_query($qU);
				}
				
				if($destino == 'PN')
				{
				$qU="update os_detalle SET cant_pickeada = 0 where ot_id=".($id_ot+0)." and id_os_detalle=".$res['id_os_detalle'];
				tep_db_query($qU);
				}
    ...
    document.location = 'monitor_ot_ps_detalle.php?id_ot=<?=$id_ot+0?>&id_estado=<?=$destino?>';
    </script>
	<?
     tep_exit();
...
function Botones($id_estado,$esta_tipo,$tipoDes,$ot_tipo){
	...
	$query_B="select id_estado_destino,id_estado_origen, desc_transicion,orden, flujo,id_estado_destino as destino from cambiosestado
...
function Flujo($otipo,$tDes,$esta_tipo,$origen){
	...
	if ($esta_tipo=='TP'){
		if (($origen=='PC')||($origen=='PG')||($origen=='PD')||($origen=='PE')||($origen=='PM')||($origen=='PN')){
			if (($otipo=='PS')&&($tDes==3)||($otipo=='PE')){
				$f=1;
/*se cambio PE por PS*/
	...
	if ($esta_tipo=='TE'){
		if (($origen=='ER')||($origen=='ED')||($origen=='EE')||($origen=='EG')|($origen=='EM')){
			if (($otipo=='PE')&&(($tDes==3) || ($tDes==5))){
				$f=1;
			}elseif (($otipo=='PS')&&( ($tDes==2)||($tDes==1) ||($tDes==6) )){
				$f=2;
			}
		}
