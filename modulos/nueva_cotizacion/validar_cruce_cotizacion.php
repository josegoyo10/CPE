...
{	
	...
	$contra = $_POST['contra'];
	$consulta = "SELECT PEMO_MOD_ID FROM permisosxmodulo WHERE PEMO_MOD_ID = " . PERMISO_CRUCE_COTIZACION . " AND PEMO_PER_ID = ( SELECT USR_ID FROM usuarios WHERE USR_LOGIN= '$usuario' AND USR_CLAVE = md5('$contra') )";
	$resultado = tep_db_query($consulta);
	$resultado_login = tep_db_fetch_array($resultado);	
	
	if(isset($resultado_login['PEMO_MOD_ID']))
	{
	$_SESSION['permiso_cruce_coti'] = 1;
	echo $_SESSION['permiso_cruce_coti'], "<br>";	
	?>
...
<form name="form1" method="post" action="validar_cruce_cotizacion.php">

<div align="center" class="titulonormal"><strong> Validaci&oacute;n de Cruce de Cotizaciones sin Pagar</strong></div>
<br>
