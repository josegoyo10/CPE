...
$USR_LOGIN = get_login_usr( $ses_usr_id );
 ...
 /* Obtiene los nombre de la persona que ingreso los datos en la OS*/
    ...
    $nc=$nombre." ".$apellido;
  	...
  	// Obtener ID de Origen WEB
    $query2 = "select id_origen FROM origenusr WHERE nom_origen='WEB'";
  	$hResult2 = tep_db_query($query2);        
    $hDataset2 = tep_db_fetch_array($hResult2);     
    $id_origen_web=$hDataset2['id_origen']; 
...
if ($accion=="dup") {
	...
	// Me fijo si el origen de la cotizacion que estoy queriendo duplicar es web
	$query="SELECT * FROM os WHERE USR_ORIGEN=".($id_origen_web+0)." AND id_os=".($id_os+0);
	$hResult3 = tep_db_query($query);        
  if(tep_db_num_rows($hResult3)==1)
  {
    // No se puede duplicar la cotizacion por que es web
    echo "<script type='text/javascript'>alert('No se puede duplicar la cotizacion por que el origen de la misma es WEB');</script>";
  }	
  else
  {
  	...
  	if ($success) {
  		...
  		header ('Location: nueva_cotizacion_sumario_01.php?id_os='.($ultimoID+0));
  	}
  	else
...
$impre="javascript:popUpWindowModal('printframe.php?id_os=$id_os', 100, 100, 710, 500);";
$impre_borra="javascript:popUpWindowModal('printframe2.php?id_os=$id_os', 100, 100, 710, 500);";
    ...
    if (($osSel['id_estado']=='SI')||($osSel['id_estado']=='SA')){
        ...
        $cod="javascript:popUpWindowModal('printcbar.php?id_os=$id_os&dire_des={dire_des}', 100, 100, 710, 500);";
    ...
    if ($osSel['id_estado']=='SE'){
     ...
     if (($osSel['id_estado']=='SA')||($osSel['id_estado']=='SI')){
        $editar='<a href="nueva_cotizacion_03.php?accion=editar&id_os='.($id_os+0).'">Editar Cotizaci&oacute;n</a>';
    ...
    // El origen es web? entonces desactivar duplicar
    if($osSel['origen']==$id_origen_web)
    {      
      $MiTemplate->set_var("duplicar_visible",'style="display:none"' );
      $duplica='';
    }    
    else
    {
      $MiTemplate->set_var("duplicar_visible",'' );      
    }
...
// Agregamos el main
$MiTemplate->set_file("main","nueva_cotizacion/nueva_cotizacion_sumario_01.htm");
...
$oses = tep_db_fetch_array( $oses );
	...
	// Si el estado es "Cotizacion" y la cotizacion esta en el rango de "Fechas validas", valida el menor precio de los productos entre la cotizacion y los productos
	...
	// Si el estado es "Cotizacion" y la cotizacion NO esta en el rango de "Fechas validas", aplica el valor actual del precio a los productos
	    ...
	    if ( $rq = tep_db_query($query_OD) ){
				...
				/* Cargamos el precio correspondiente al precio actual del producto */
	                ...
	                if($res['osde_tipoprod'] == 'PS')
	                {
	                $queryOD ="UPDATE os_detalle OD INNER JOIN productos P ON OD.cod_sap = P.cod_prod1
	       						SET osde_precio='".$new_Precio."' where id_os_detalle=".($idOS_detalle+0)."";
   		   			tep_db_query($queryOD);    
	                }
	...
	// Si el estado NO es "Cotizacion" y la cotizacion NO esta en el rango de "Fechas validas", no aplica cambios al precio de los productos
 	else{
