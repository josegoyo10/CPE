...
if ($tipocod == "UPC") { //cod_barra//UPC//EAN
	if ( is_numeric(trim($sku))) //es número
		$query = "SELECT pr.*, if (pr.prod_tipo = 'PE' and pr.prod_subtipo = 'GE', null, p.prec_valor) prec_valor, p.prec_costo, cb.cod_barra, precio_req, valid_stock, precio_mod FROM codbarra cb LEFT JOIN productos pr ON pr.cod_prod1 = cb.cod_prod1 LEFT JOIN precios p ON p.cod_prod1 = pr.cod_prod1 AND id_local = " . ($usr_local+0) . " JOIN tipo_subtipo tst ON tst.prod_tipo = pr.prod_tipo AND tst.prod_subtipo = pr.prod_subtipo WHERE cb.unid_med not in ('PAL','TON','T') and cb.cod_barra ='" . trim($sku) ."' and pr.estadoactivo='C' ORDER BY cod_ppal desc";
	else // es texto
		$query = "SELECT pr.*, if (pr.prod_tipo = 'PE' and pr.prod_subtipo = 'GE', null, p.prec_valor) prec_valor, p.prec_costo, cb.cod_barra, precio_req, valid_stock, precio_mod FROM codbarra cb LEFT JOIN productos pr ON pr.id_producto = cb.id_producto LEFT JOIN precios p ON p.id_producto = pr.id_producto AND id_local = " . ($usr_local+0) . " JOIN tipo_subtipo tst ON tst.prod_tipo = pr.prod_tipo AND tst.prod_subtipo = pr.prod_subtipo WHERE cb.unid_med not in ('PAL','TON','T') and  cb.cod_barra = '" . trim($sku) . "' and pr.estadoactivo='C' ORDER BY cod_ppal desc" ;
}
else { //código SKU//SAP//COD_PROD1
	if ( is_numeric(trim($sku))) //es número
		$query = "SELECT pr.*, if (pr.prod_tipo = 'PE' and pr.prod_subtipo = 'GE', null, p.prec_valor) prec_valor, p.prec_costo, cb.cod_barra, precio_req, valid_stock, precio_mod, cod_ppal, unid_med FROM productos pr LEFT JOIN codbarra cb ON pr.cod_prod1 = cb.cod_prod1 LEFT JOIN precios p ON p.cod_prod1 = pr.cod_prod1 AND id_local = " . ($usr_local+0) . " JOIN tipo_subtipo tst ON tst.prod_tipo = pr.prod_tipo AND tst.prod_subtipo = pr.prod_subtipo WHERE  cb.unid_med not in ('PAL','TON','T') and  pr.cod_prod1 ='" . trim($sku) . "' and pr.estadoactivo='C' ORDER BY cod_ppal desc";

	else // es texto
		$query = "SELECT pr.*, if (pr.prod_tipo = 'PE' and pr.prod_subtipo = 'GE', null, p.prec_valor) prec_valor, p.prec_costo, cb.cod_barra, precio_req, valid_stock, precio_mod, cod_ppal, unid_med FROM productos pr LEFT JOIN codbarra cb ON pr.cod_prod1 = cb.cod_prod1 LEFT JOIN precios p ON p.cod_prod1 = pr.cod_prod1 AND id_local = " . ($usr_local+0) . " JOIN tipo_subtipo tst ON tst.prod_tipo = pr.prod_tipo AND tst.prod_subtipo = pr.prod_subtipo WHERE pr.cod_prod1 = '" . trim($sku) . "' and pr.estadoactivo='C' ORDER BY cod_ppal desc";
}
