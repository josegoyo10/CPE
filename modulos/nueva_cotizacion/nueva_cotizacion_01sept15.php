...
if( $response[0]['IdTypeCustomer']==0 || $response[0]['IdTypeCustomer']==2 || $response[0]['IdTypeCustomer']==4 || $response[0]['IdTypeCustomer']==6 || $response[0]['IdTypeCustomer']==8 || $response[0]['IdTypeCustomer']==9 || $response[0]['IdTypeCustomer']==10 || $response[0]['IdTypeCustomer']==11 || $response[0]['IdTypeCustomer']==12 )
{ 
$xml = "<request>
  <customer>
    <Address>$dire_direccion</Address>
...
$MiTemplate->set_file("main4","nueva_cotizacion_botones.html");
	...
	$query = "SELECT id_categoria, nombre_categoria,  case WHEN clie_rut is null then '' else 'selected' end selected   FROM  tipo_categoria_cliente T LEFT JOIN clientes C ON T.id_categoria = C.clie_categoria_cliente AND C.clie_rut = " . $eclient['clie_rut'];
	}    
