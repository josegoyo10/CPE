...
function popUpWindowMod(URLStr, left, top, width, height)
{  
  window.open(URLStr, 'Ajustar', 'toolbar=NO, directories=NO, status=YES, scrollbars=YES width='+width+',height='+height+',left='+left+', top='+top);  
}
...
function popUpWindowModalPicking(URLStr, cadena)
{
  popUpWin = showModalDialog(URLStr + '?id_ot=' + cadena, 'popUpWin', 'dialogHeight: ' + 'auto' + 'px; dialogWidth: 667px; help: no; scroll: no; status: no ; status bar: no ;copyhistory=yes');
  //document.location.reload();
  return popUpWin;
}

function popUpWindowModalGuia(URLStr,cadena)
{
  popUpWin = showModalDialog(URLStr + '?id_ot=' + cadena, 'popUpWin', 'dialogHeight: ' + 'auto' + 'px; dialogWidth: 667px; help: no; scroll: no; status: no ; status bar: no ;copyhistory=yes');
  //document.location.reload();
  return popUpWin;
