    ...
    if ( $rq = tep_db_query($qzyCant) ){
        while( $res = tep_db_fetch_array( $rq ) ) {
	        ...
	        {        	
        		...
        		echo "alert('No Hay ningún resultado para esta Búsqueda');";
        		echo "</script>";
        	}
...
function listado_producto() {
    ...
    if ( $rad_producto == 1 ) // Se busca por codigo SAP
        $where_aux = " and p.cod_prod1 = '$producto' and p.estadoactivo='C' AND pre.estadoactivo='C' AND cb.estadoactivo='C' ";
    else if ( $rad_producto == 2 ) // Se busca por UPC
        $where_aux = " and cb.cod_barra = '$producto' and p.estadoactivo='C' AND pre.estadoactivo='C' AND cb.estadoactivo='C' ";
    else if ( $rad_producto == 3 ) // Se busca por descripcion
        $where_aux = " and MATCH(p.des_larga) AGAINST ('$producto') and p.estadoactivo='C' AND pre.estadoactivo='C' AND cb.estadoactivo='C' ";
    ...
    $qzyCantp = "SELECT count(DISTINCT p.id_producto) AS cuantosp
            FROM productos p
            JOIN precios pre
...
function listado_servicio() {
    ...
    if ( $rad_servicio == 1 ) // Se busca por codigo SAP
        $where_aux = " and p.cod_prod1 = '$servicio' and p.estadoactivo='C' AND pre.estadoactivo='C' AND cb.estadoactivo='C' ";
    else if ( $rad_servicio == 2 ) // Se busca por UPC
        $where_aux = " and cb.cod_barra = '$servicio' and p.estadoactivo='C' AND pre.estadoactivo='C' AND cb.estadoactivo='C' ";
    else if ( $rad_servicio == 3 ) // Se busca por descripcion
        $where_aux = " and p.des_corta like '%$servicio%' and p.estadoactivo='C' AND pre.estadoactivo='C' AND cb.estadoactivo='C' ";
...
function ver_productos(){
    ...
    $query_ver ="SELECT p.id_producto,   
					...
					pre.prec_valor  
                FROM prodxprov pp 
					/*LEFT*/ JOIN precios pre ON pre.cod_prod1 = pp.cod_prod1 AND id_local = ".get_local_usr($ses_usr_id)."
					JOIN productos p ON p.cod_prod1 = pp.cod_prod1
				WHERE p.estadoactivo='C' AND pre.estadoactivo='C'  and pp.id_proveedor = ".($id_prov + 0)." limit 0," . LIMITE_REG;
...
function ver_ficha() {
    ...
    $queryP = "select p.id_producto, 
                ...
                where p.estadoactivo='C' and p.id_producto=".($id_pro+0)." limit 0," . LIMITE_REG;
...
function listado() {
	...
	$query_SV ="SELECT distinct p.id_producto, 
					...
					p.prod_tipo 
				FROM productos p 
				JOIN precios pre on pre.cod_prod1 = p.cod_prod1
				JOIN codbarra cb on cb.cod_prod1 = p.cod_prod1
				WHERE p.estadoactivo='C' AND pre.estadoactivo='C' AND cb.estadoactivo='C'  and prod_tipo = 'SV' 
...
function listado_ape() {
	...
	$query_SV =" SELECT distinct p.id_producto, 
				...
				AND id_local = ".get_local_usr($ses_usr_id)." 
				JOIN codbarra cb on cb.cod_prod1 = p.cod_prod1
				WHERE p.estadoactivo='C' AND pre.estadoactivo='C' AND cb.estadoactivo='C' and prod_tipo = 'PE' 
				AND prod_subtipo in (".$subtipo.") order by des_larga limit 0,200" ; 
	// para el boton de volver o cerrar
...
if( $accion == 'PE' )
    ...
    ver_ficha_pro();
else if( $accion == 'despachos' || $accion == 'instalaciones' || $accion == 'visitas' || $accion == 'arriendo' || $accion == 'seguro')
    listado();
