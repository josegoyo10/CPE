<?php
$pag_ini = "../nueva_cotizacion/seleccionar_producto.php";
...
$queryClie = "SELECT  DISTINCT P.id_producto, P.cod_prod1, P.des_larga, PV.cod_prov, PRV.nom_prov, F.numero_dias FROM productos P
			LEFT JOIN prodxprov PV ON P.id_producto = PV.id_producto
			LEFT JOIN proveedores PRV ON PV.cod_prov = PRV.cod_prov
			LEFT JOIN fecha_entrega  F ON P.cod_prod1 = F.cod_sap AND PRV.cod_prov = F.cod_proveedor
			JOIN precios PR ON P.cod_prod1 = PR.cod_prod1
			JOIN codbarra CB ON P.cod_prod1 = CB.cod_prod1 
			WHERE P.prod_tipo = 'PE'  
			AND CB.estadoactivo = 'C' AND  P.estadoactivo = 'C'  AND  PR.estadoactivo = 'C' ";	
...
//PV.cod_prov <> 'INT_C009'
	...
	if($numero == 0)
	{
		...
		echo "alert('No existen Productos PE creados para la consulta solicitada')";
		echo "</script>";
	}
...
var descProd2 = '';
var numDias2 = '';
function concatenar(codSap, nomProv, codProv, codProd, descProd, numDias)
{
codSap2 = codSap;
nomProv2 = nomProv;
codProv2 = codProv;
...
function colocar_valor()
{
	...
	window.opener.formulario.codProd.value = codProd2;
...
<body>
<p align="center" class="titulonormal"><strong>BUSQUEDA DE PRODUCTO PE </strong></p>
<form name="forma" method="post" action="seleccionar_producto.php" onSubmit="return validar();">
...
<table width="788" border="1" align="center" class="tabla1">
  <tr class="textoespecial1">
    <td width="64"><div align="left" class="tabla2">Id Producto </div></td>
    <td width="66"><div align="left" class="tabla2">C&oacute;digo Sap </div></td>
    <td width="302"><div align="left" class="tabla2">Descripci&oacute;n Producto </div></td>
