...
/* Realiza la busqueda de los datos del cliente*/
    $queryClie =  "SELECT clie_rut, clie_nombre, clie_paterno, clie_materno, clie_email, clie_telefonocasa,clie_telcontacto1,clie_telcontacto2 
    FROM clientes where clie_rut=".($clie_rut+0)." ";


file_put_contents('sqlCLIEntE.txt', print_r($queryClie,true));
    ...
    file_put_contents('rutE.txt', print_r($eclient,true));
    ...
    else
    {
        ...
        file_put_contents('ingreso.txt', 'insert');
...
if ($accion=="update"){
    ...
    $exis = tep_db_fetch_array( $exis );
     ...
     //$num = strlen($barrios);
    file_put_contents('localizarBarrio01.txt', $barrios);


    //Se agrega esta validacion J.G 14-12-2018 
   if ($barrios != '') {
    	$localizacion = $barrios;
    	 $num = strlen($barrios);
    } else {
       $num = strlen($barrio_local);
       
       // Se adiciona esta query para consultar datos del cliente..JG 17-12-2018
        $sqlClient 			="SELECT *FROM clientes WHERE clie_rut=".($clie_rut+0);
        $row 		  		= tep_db_query($sqlClient);
        $resultados   		= tep_db_fetch_array($row);
        //  file_put_contents('resultQuery.txt', print_r($resultados));
        $localizacion 		= $barrio_local;
        $clie_nombre  		= $resultados['clie_nombre'];
        $clie_paterno  		= $resultados['clie_paterno'];
        $clie_materno  		= $resultados['clie_materno'];
        $clie_telefonocasa  = $resultados['clie_telefonocasa'];
        $clie_telcontacto1  = $resultados['clie_telcontacto1'];
        $clie_telcontacto2  = $resultados['clie_telcontacto2'];
        $clie_email         = $resultados['clie_email'];
        $sexo         		= $resultados['clie_sexo'];
        $tipo_cliente       = $resultados['clie_tipo_cliente'];
        $categoria_cliente  = $resultados['clie_categoria_cliente'];
        $tipo_contribuyente = $resultados['tipo_contribuyente'];
        $departamento       = $resultados['clie_departamento'];
        $id_tipo_doc        = $resultados['id_tipo_doc'];
        $dire_direccion     = $direccion_cliente;
    ...
    file_put_contents('localizacionBarrio02.txt', $localizacion);

   //Se le agrego este campo J.G 14-12-2018
    ...
    $queryUpC ="UPDATE clientes SET clie_nombre='$clie_nombre', clie_paterno='$clie_paterno', clie_materno='$clie_materno',clie_telefonocasa='$clie_telefonocasa',clie_telcontacto1='$clie_telcontacto1',clie_telcontacto2='$clie_telcontacto2',clie_activo=1,clie_tipo='p', clie_email='$clie_email', clie_ciudad = $ciudad, clie_barrio = $barrios, clie_sexo = '$sexo', clie_tipo_cliente = $tipo_cliente, clie_categoria_cliente = $categoria_cliente, clie_tipo_contribuyente = '$tipo_contribuyente', clie_departamento = $departamento, clie_localidad = $clie_localidad,  clie_provincia = $clie_provincia, clie_localizacion = $localizacion, id_tipo_doc = $id_tipo_doc WHERE clie_rut=".($clie_rut+0).""; 
      ...
      file_put_contents('queryUPC.txt',print_r($queryUpC,true));
    ...
    tep_db_query($queryUpC);
             ...
             if ($edirec['dire_direccion']!='' ){
                $queryUpD ="UPDATE direcciones SET dire_direccion='$dire_direccion',dire_telefono='$clie_telefonocasa',dire_observacion='$dire_observacion', id_departamento = $departamento, id_ciudad = $ciudad, id_localidad = $clie_localidad, id_comuna= $barrios,  id_provincia = $clie_provincia, dire_localizacion = $localizacion  WHERE clie_rut=" . $clie_rut . " and dire_defecto='p'";
                 ...
                 $queryUpD =  "INSERT INTO direcciones 
                   (dire_direccion,clie_rut,dire_telefono,dire_observacion, id_departamento, id_ciudad, id_localidad, id_comuna, dire_activo,dire_defecto,dire_nombre, id_provincia, dire_localizacion ) 
                    VALUES('$dire_direccion',".($clie_rut+0).",'$clie_telefonocasa',
                     '$dire_observacion', $departamento, $ciudad, $clie_localidad, 
                     $barrios, 1,'p','Principal', $clie_provincia, $localizacion)";
            }
...
/* MODO BUSQUEDA PARA CLIENTE UNICO*/        
   ...
   $response = ClientUnique::searchById($clie_rut);
		if($response){
			 file_put_contents('exitoWs.txt', $response);
			}
		else {
			 file_put_contents('errorWs.txt', $response);
...
if ( isset($rutE) ){
	    ...
	    $datosCliente = ClientUnique::searchById($clie_rut);		
          ...
          file_put_contents('datosCliente.txt', print_r($datosCliente,true));

		if($datosCliente['IdCustomer'] != 0)
		{

            file_put_contents('datosClienteID.txt', $datosCliente['IdCustomer']);
