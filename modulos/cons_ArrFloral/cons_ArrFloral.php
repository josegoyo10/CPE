<?
$pag_ini = '../cons_ArrFloral/cons_ArrFloral.php';
...
global $ses_usr_id;   
    ...
    // Agregamos el main
    $MiTemplate->set_file("main","cons_ArrFloral/cons_ArrFloral.htm");  
    ...
    // Escribe el encabezado de la Tabla según los criterios de Consulta establecidos
    	if (!$fecha_desde && !$fecha_hasta && $idCentroDespacho)
					$glosa = "Pedidos de Arreglos Florales  para el Centro de Despacho $nom_local";
				elseif($fecha_desde && !$fecha_hasta && $idCentroDespacho)
					$glosa = "Pedidos de Arreglos Florales desde el $fecha_desde para el Centro de Despacho $nom_local";
				elseif(!$fecha_desde && $fecha_hasta && $idCentroDespacho)
					$glosa = "Pedidos de Arreglos Florales hasta el $fecha_hasta para el Centro de Despacho $nom_local";
				elseif($fecha_desde && $fecha_hasta && $idCentroDespacho)
					$glosa = "Pedidos de Arreglos Florales entre el $fecha_desde y el $fecha_hasta para el Centro de Despacho $nom_local";
				elseif(!$idCentroDespacho && !$fecha_hasta && $fecha_desde)
					$glosa = "Pedidos de Arreglos Florales para todos los Centros de Despacho desde el $fecha_desde";
				elseif(!$idCentroDespacho && !$fecha_desde && $fecha_hasta)
					$glosa = "Pedidos de Arreglos Florales para todos los Centros de Despacho hasta el $fecha_hasta";
				elseif(!$idCentroDespacho && $fecha_desde && $fecha_hasta)
					$glosa = "Pedidos de Arreglos Florales para todos los Centros de Despacho desde el $fecha_desde hasta el $fecha_hasta";
				elseif(!$idCentroDespacho && !$fecha_desde && !$fecha_hasta)
					$glosa = "Pedidos de Arreglos Florales para todos los Centros de Despacho";
	...
	$queryCount="SELECT DISTINCTROW COUNT(*) AS Cont
				 ...
				 WHERE 1
				 AND OT.ot_tipo='PE' AND O.origen = 'V'
				 ".(($idEstado)?" AND OT.id_estado ="."'".$idEstado."'":"")."
	...
	if($tot_reg == 0){
		$MiTemplate->set_var("alert","<script language=\"JavaScript\">alert(\"No existen pedidos de Arreglos Florales para la consulta solicitada. \");</script>");		
	}
			
	else{
		$queryRES = "SELECT DISTINCTROW OT.ot_tipo, E.esta_nombre, OT.ot_id, OT.noc_sap, OD.cod_sap, OD.osde_precio, OD.osde_cantidad, OD.cod_barra, P.des_larga, OD.osde_fecha_entrega, UPPER(O.os_comentarios) AS observacion, O.os_fechacreacion, L.nom_local,
						CL.clie_rut, CONCAT(CL.clie_nombre,' ',CL.clie_paterno,' ',CL.clie_materno) AS nom_cliente, CL.clie_telefonocasa, CL.clie_telcontacto2
						FROM os_detalle OD
						LEFT JOIN os O ON O.id_os= OD.id_os
						LEFT JOIN ot OT ON OT.id_os = O.id_os
						LEFT JOIN productos P ON P.cod_prod1 = OD.cod_sap
				...
				$MiTemplate->set_var("clie_cel", ($res['clie_telcontacto2'])?$res['clie_telcontacto2']:"<br>");
