...
///////////////////// ACCIONES //////////////////////
if ($accion=='ANULAR'){	
	if($id_estado == 'ET' || $id_estado == 'EC' || $id_estado == 'ER' || $id_estado == 'EP')
	{
	$actualiza_OT = "UPDATE ot SET  id_estado = 'EN' WHERE ot_id = $id_ot ";    
	$Resultado_OT = tep_db_query($actualiza_OT);    
	}
...
if (($accion=='CambEst') && ($id_tipodespacho!=3) && ($id_tipodespacho!=5) &&($desp_ddpr==1) && (($destino=='ET') || ($destino=='EM'))  ){
	cambia_estado($id_ot,$destino,$transicion,1,$accion,$id_estado); 
}
if (($accion=='CambEst') && ($id_tipodespacho!=3) && ($id_tipodespacho!=5) && ($desp_ddpr!=1)&&($destino!='EM')){
	...
	if (!$resultado_ws){
		...
		if ($resultado_ws){
			...
			cambia_estado($id_ot,'EM',$tipoDes,$id_estado,$esta_tipo,$accion,$ot_tipo);
		}

}
if (($accion=='CambEst') && (($id_tipodespacho==3) || ($id_tipodespacho==5))){
...
// Agregamos el main
$MiTemplate->set_file("main","monitor_ot_pe/monitor_detalle_01.htm");

//################################### DATOS OT ####################################
$MiTemplate->set_block("main", "datosot", "LIS_datosot");
$query="SELECT	ot.ot_id, noc_sap,
				esta_nombre, 
				t.nombre tipo_despacho, t.id_tipodespacho,
				date_format(ot_freactivacion, '%d/%m/%Y') as fechareac, DATE_FORMAT(od.osde_fecha_entrega, '%e/%m/%Y') as osde_fecha_entrega,
...
$query="SELECT	D.dire_direccion dire_direccion_2,
				...
				dire_telefono AS dire_telefono_2
		FROM ot join os on os.id_os = ot.id_os 
...
if (($res["id_estado"]=='EC')&&(!$idp)){
	...
	$MiTemplate->set_var('endcomment',$endC);
	 ...
	 //Recuperamos proveedor
		...
		$MiTemplate->set_var('razsoc_prov',$res["nom_prov"]);
...
if ($res['desp_ddp'])
	$flujo = 3;
elseif ($res['id_tipodespacho'] == '3' || $res['id_tipodespacho'] == '5')
	$flujo = 1;
else 
	$flujo = 2;
