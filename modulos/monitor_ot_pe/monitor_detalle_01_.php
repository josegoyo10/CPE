...
require_once('../monitor_despacho/wsgdclient.php');
///////////////////// ACCIONES //////////////////////
if ($accion=='AgrTr'){
...
if ($accion=='CambEst'){
	...
	//writelog('$resultado_ws '.$resultado_ws );
	if ($resultado_ws==0){
		//error
		//echo("XML respuesta no ok:[$resultado_ws]");
		?><script language="JavaScript">
        window.alert("Existe un problema al enviar la OD a Despacho a Domicilio por Favor comuniquese con la Mesa de Ayuda \n\t<?=$msgerror?>");
        </script>
		<?
...
if (($res["id_estado"]=='EC')&&(!$idp)){
	...
	$MiTemplate->set_var('endcomment',$endC);
	 ...
	 //Recuperamos proveedor
		$query_PR = "SELECT pv.id_proveedor, pv.rut_prov,pv.razsoc_prov,pv.nom_prov, ex.nomcontacto,pv.fonocto_prov,ex.mailcontacto,pp.cod_prod1 
		FROM proveedores pv
...
$queryHist="SELECT distinct desc_transicion, ce.orden,flujo,id_estado_destino, estadoterminal 
