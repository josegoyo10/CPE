<?
$pag_ini = '../monitor_ot_pe/monitor_ot_pe_ca.php';
...
function DisplayListado() {
    ...
    // Agregamos el main
    $MiTemplate->set_file("main","monitor_ot_pe/monitor_ot_pe_ca.htm");
	...
	$query_A="SELECT distinct date_format(ot_fechacreacion, '%Y') as fecha, if (DATE_FORMAT( ot_fechacreacion, '%Y')='$anoselect', 'selected', '') selected 
			...
			WHERE ot_fechacreacion is not null and ot_tipo in ('PE') $where_aux_local 
			ORDER BY 1";
...
/*///////////////////////////////////////////////////para la paginacion////////////////////////////////////////*/    
	...
	$query_count = "SELECT count(*) as cuenta 
			FROM ot JOIN os on os.id_os = ot.id_os 
					JOIN os_detalle od on od.ot_id = ot.ot_id 
			...
			WHERE ot_tipo in ('PE') and od.osde_subtipoprod='CA' $where0 $where1 $where2 $where3 $where4 $where6 $where7 $where8 $where9 $where10 $where_aux_local";
...
/*//////////////////////////////////////////////////////////////////////////////////////////*/    
	...
	$query="SELECT ot.ot_id, ot.id_os as id_os, nom_local,date_format(ot_fechacreacion, '%d/%m/%Y  %H:%m:%s') ot_fechacreacion, date_format(ot_freactivacion, '%d/%m/%Y') ot_freactivacion, esta_nombre, if (e.estadoterminal = 1, 'green', if (ot_freactivacion < adddate( now(), -1), 'red', 'black')) color,ot.id_estado as estado 
			FROM ot JOIN os on os.id_os = ot.id_os 
					JOIN os_detalle od on od.ot_id = ot.ot_id 
					JOIN estados e on e.id_estado = ot.id_estado 
					JOIN locales l on l.id_local = os.id_local 
					JOIN tipos_despacho td on td.id_tipodespacho = ot.id_tipodespacho 
			WHERE ot_tipo in ('PE') and od.osde_subtipoprod='CA' $where0 $where1 $where2 $where3 $where4 $where6 $where7 $where8 $where9 $where10 $where_aux_local $order1 LIMIT ".($desde+0).",".($limite+0);
   ...
   //Recuperamos las fechas de creacion
	...
	$queryf="SELECT distinct date_format(ot_fechacreacion, '%Y-%m-%d') as fechat, DATE_FORMAT( ot_fechacreacion, '%d/%m/%Y') as fecha , if (DATE_FORMAT( ot_fechacreacion, '%Y-%m-%d')='$select_fecha', 'selected', '') selected 
			FROM ot JOIN os on os.id_os = ot.id_os 
					JOIN estados e on e.id_estado = ot.id_estado 
			WHERE ot_fechacreacion is not null and ot_tipo in ('PE') $where0 $where1 $where2 $where3 $where7 $where8 $where9 $where10 $where_aux_local 
			ORDER BY 1";
   ...
   //Recuperamos las fechas de reactivacion
	...
	$queryf="SELECT distinct date_format(ot_freactivacion, '%Y-%m-%d') as fechat, DATE_FORMAT( ot_freactivacion, '%d/%m/%Y') as fecha , if (DATE_FORMAT( ot_freactivacion, '%Y-%m-%d')='$select_fechar', 'selected', '') selected 
			FROM ot JOIN os on os.id_os = ot.id_os 
					JOIN estados e on e.id_estado = ot.id_estado 
			WHERE ot_freactivacion is not null and ot_tipo in ('PE') $where0 $where1 $where2 $where3 $where7 $where8 $where9 $where10 $where_aux_local 
			ORDER BY 1";
