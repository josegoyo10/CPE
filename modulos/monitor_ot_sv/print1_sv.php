...
// *************************************************************************************
$qry=" SELECT DISTINCT o.id_os,date_format(ot_fechacreacion, '%d/%m/%Y ')ot_fechacreacion ,e.esta_nombre
	FROM os_detalle o 
...
/* acciones*/
$querysel="SELECT CL.clie_tipo,CL.clie_razonsocial,S.os_fechaestimacion,CL.clie_rut,S.os_descripcion,S.os_comentarios ,
			S.id_os ,S.clie_rut ,CL.clie_nombre,CL.clie_paterno,CL.clie_materno,CL.clie_telefonocasa,L.nom_local,
			S.os_fechacotizacion ,E.esta_nombre,P.proy_nombre,D.dire_direccion,D.dire_observacion,D.dire_telefono,cod_local_pos
			from os S
...
$dirCli = consulta_localizacion($osSel['clie_rut'],1);
...
$MiTemplate->set_var("USR_NOMBRE",get_nombre_usr( $ses_usr_id ));
     ...
     $MiTemplate->set_var("comu_nombre",$dirCliente['barrio']." - ".$dirCliente['localidad']);

     $MiTemplate->set_var("dire_telefono_des",$osSel['dire_telefono']);
     $MiTemplate->set_var("dire_direccion_des",$osSel['dire_direccion']);
     $MiTemplate->set_var("departamento_des",$dirServicio['departamento']);
     $MiTemplate->set_var("ciudad_des",$dirServicio['ciudad']);
     $MiTemplate->set_var("comu_nombre_des",$dirServicio['barrio']." - ".$dirServicio['localidad']);
     $MiTemplate->set_var("dire_observacion_des",$osSel['dire_observacion']);



if($dire_des){
     $MiTemplate->set_var("dire_telefono_des",$osSelDire['dire_telefono_des']);
     $MiTemplate->set_var("dire_direccion_des",$osSelDire['dire_direccion_des']);
     $MiTemplate->set_var("comu_nombre_des",$dirServicio['barrio']." - ".$dirServicio['localidad']);
     $MiTemplate->set_var("dire_observacion_des",$osSelDire['dire_observacion_des']);
}
// Agregamos el main
$MiTemplate->set_file("main","monitor_ot_sv/print1.htm");
...
/* para los productos*/
$MiTemplate->set_file("productos","monitor_ot_sv/os_detalle.htm");
/* para los productos*/
$instn="No";
$insts="Sí";
$precioVacio=" - ";
