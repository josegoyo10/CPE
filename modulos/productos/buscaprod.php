...
if (isset($codigo) && is_numeric($codigo) && $codigo>0 && isset($tipocod) && ($tipocod=="SAP" || $tipocod=="EAN")) {
    ...
    if ($tipocod=="SAP") 
        $Mquery = "SELECT des_corta FROM productos where 
        estadoactivo = 'C' AND 
        cod_prod1 = '$codigo'";

    if ($tipocod=="EAN") 
        $Mquery = "SELECT des_corta FROM codbarra c JOIN productos p on p.cod_prod1 = c.cod_prod1 WHERE  
        p.estadoactivo = 'C' AND c.estadoactivo = 'C' AND 
        c.cod_barra = '$codigo'";
