...
// para cuando viene de generar facturacion
if(($id_lote)){
	// Agregamos el main
	$MiTemplate->set_file("main","facturacion/facturacion_03.htm");
	/*para el detalle de los servicios prefacturados*/
		$MiTemplate->set_block("main", "detalle_lote", "blo_lote");
		...
		where O.id_lote=".$id_lote." order by O.ot_id";
		if ( $rq = tep_db_query($qeryfd) ){
			while( $res = tep_db_fetch_array( $rq ) ) {
				...
				$MiTemplate->parse("blo_lote", "detalle_lote", true);
			}
		}
	...
	$qry_fac="select id_lote,estado,fechageneracion,usuario,monto_factura,retencion_lote,num_factura from lote_instalador where id_lote=".$id_lote;
		...
		$MiTemplate->set_var('usuarioingreso',$res1["usuario"]);
...
if ($accion=='Deshacer'){
	/* cambia de estado la ot de VM a VP se agregan los margenes a pagar*/
	$qeryot="UPDATE ot SET margenapagar=null ,subtotalapagar= null, id_lote= null ,id_estado='VM' where id_lote=".($id_lote+0);
	tep_db_query($qeryot);
	insertahistorial("El lote de facturacion $id_lote fue desecho , dejando sus cantidades en nulo y cambiando el estado de Pre-Facturado a Realizado");
	?>
    <script language="JavaScript">
	    document.location = 'facturacion_01.php?id_instalador=<?=$id_instalador+0?>';
    </script>
	<?
	tep_exit();
}
