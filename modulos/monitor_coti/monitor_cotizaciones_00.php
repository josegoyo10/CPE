<?
$pag_ini = '../monitor_coti/monitor_cotizaciones_00.php';
...
function listado_cotizaciones() {
    ...
    // Agregamos el main
    $MiTemplate->set_file("main","monitor_coti/monitor_cotizaciones_00.html");
	...
	*/
	...
	if ($Ori_Gen){
		$aux_ori= "and o.USR_ORIGEN=".$Ori_Gen;
	}else{
		$aux_ori= " ";
	}
	...
	$query_count = "SELECT count(*) as cuenta 
			...
			WHERE origen in ('C','S','V')
			$where_aux_local $aux_ori $where_aux  $where_auxl  $where_auxe $order_aux";
    ...
    $query = "SELECT distinct o.id_os, o.os_descripcion, date_format( o.os_fechacreacion, '".DATE_FORMAT_S."' ) as os_fechacreacion, c.clie_rut, c.clie_nombre, c.clie_paterno, c.clie_telcontacto2, c.clie_telefonocasa, e.esta_nombre
            FROM os o
            join clientes c on c.clie_rut = o.clie_rut
            join estados e on e.id_estado = o.id_estado and o.id_os > 0 
			join usuarios U on o.USR_ORIGEN=U.USR_ORIGEN
			WHERE origen in ('C','S','V')
			$where_aux_local $aux_ori $where_aux  $where_auxl $where_auxe  $order_aux  LIMIT ".($desde+0).",".($limite+0);
