...
/* si la direccion ya existe*/
        $queryclie="SELECT CL.clie_rut, CL.clie_nombre, CL.clie_paterno FROM clientes CL
					INNER JOIN list_regalos_enc L on (CL.clie_rut=L.clie_Rut) where L.idLista =".($idLista+0)."";
    ...
    if ($id_direccion>0){
        ...
        $qupdi="UPDATE list_regalos_enc SET id_Direccion = " . $id_direccion . " where idLista = " . $idLista;
        tep_db_query($qupdi);
    }else{
       /* inserta la direccion y la deja como primaria*/
        ...
        /* cambia el id de la direccion en la Lista de Regalos*/
        $qupdi="UPDATE list_regalos_enc SET id_Direccion=".($UidistD+0)." where idLista=".($idLista+0)." ";
        tep_db_query($qupdi);
     }
...
if($accion=="Nueva"){
    $queryclie="SELECT CL.clie_nombre,CL.clie_paterno FROM clientes CL 
                INNER JOIN list_regalos_enc L ON (CL.clie_rut=L.clie_Rut) WHERE L.idLista =".($idLista+0)."";   
    $clien = tep_db_query($queryclie);
    $clien = tep_db_fetch_array($clien);
}
...
// Agregamos el main
$MiTemplate->set_file("main","lista_Regalos/libreta2.htm");
