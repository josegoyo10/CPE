...
include_idioma_mod( $ses_idioma, "nueva_lista_00" );
   ...
   if ($tpcliente=="persona") 
            ...
            if ($ingresadocomo == 'p'){
            	?>
                    ...
                    location.href='nueva_lista_01.php?clie_rut=<?=$eclien['clie_rut']?>' ;
                    </script>
                <?php
                }
            if ($ingresadocomo == ''){
   					...
   					if($response['IdCustomer']){
						...
						if( $response['IdGroup']==1 || $response['IdGroup']== "")
						{
							if(validarcliente($response)){
								?>
									<script language="JavaScript">
	                    				location.href='nueva_lista_01.php?clie_rut=<?=$response['IdCustomer']?>' ;
	                    			</script>
	                    		<?php
							}
						...
						if( $response['IdGroup'] == 2)
						{
							...
							if(confirm('El Cliente no se encuentra creado en Cliente Único. \n\t        Desea ingresarlo?'))
								location.href='nueva_lista_01.php?clie_rut=<?=$clie_rut?>' ;
							</script>
							<?php
					}
				}
...
// Agregamos el main
$MiTemplate->set_file("main","lista_Regalos/nueva_lista_00.htm");
