...
include_idioma_mod( $ses_idioma, "monitor_OTRegalos" );
...
function listado_cotizaciones() {
    ...
    // Agregamos el main
    $MiTemplate->set_file("main","cotiza_Regalos/monitor_OTRegalos.html");    
    ...
    if($select_local)
        $where_auxl = " AND LO.ot_listTiendaPago = ".( $select_local + 0 ); 
    if($rad_selec == 1 && $bus_List)
            $where_aux = " AND LO.ot_idList = ".( $bus_List + 0 );
    else if($rad_selec == 2 && $bus_List)
            $where_aux = " LE.invitado like '%$bus_List%'";                   
            
    if ( $orden == "" || $orden == "asc"){
    	$order_aux = "ORDER BY LO.ot_idList $orden";
    ...
    /************************************para la paginacion***********************************/    
		...
		$query_count = "SELECT count(*) as cuenta FROM list_os_det LD
				         LEFT JOIN list_os_enc LE ON LE.idLista_OS_enc = LD.idLista_OS_enc
				         LEFT JOIN list_ot LO ON LO.ot_idList =LD.OS_idOT
				         LEFT JOIN estados E ON E.id_estado = LO.ot_idestado
					    WHERE LD.OS_idOT IS NOT NULL AND LE.idLista_enc= $idLista
    ...
    $query = "SELECT DISTINCT LO.ot_idList, date_format(LO.ot_listFeccrea, '%d/%m/%Y' ) AS fecCrea, E.esta_nombre, LO.ot_listTiendaPago, L.nom_local, LE.invitado, LD.idLista_OS_enc  FROM list_os_det LD
		         LEFT JOIN list_os_enc LE ON LE.idLista_OS_enc = LD.idLista_OS_enc
		         LEFT JOIN list_ot LO ON LO.ot_idList =LD.OS_idOT
		         LEFT JOIN estados E ON E.id_estado = LO.ot_idestado
		         LEFT JOIN locales L ON L.id_local = LO.ot_listTiendaPago
			WHERE LD.OS_idOT IS NOT NULL AND LE.idLista_enc= $idLista
			$where_aux  $where_auxl  $order_aux LIMIT ".($desde+0).",".($limite+0);		
