 ...
 *                    Kristian Koehntopp
 *
 * $Id: template.php,v 1.1 2008/06/04 14:25:08 tvarela Exp $
 *
 */
...
class Template
{
  ...
  */
  function subst($varname) {
    ...
    //Modificación RGM 24/03/2003. Debemos buscar si existe alguna aparición de una variable del estilo {varname(N)} y si es así, tratarla de forma especial para rellenar
    $nstr = $str;
    while ($nstr) {
        if (ereg("(\{([a-zA-Z0-9_]+)\(([0-9]+)\)\((i|d)\)\})", $nstr, $regs)) {
            //$regs[1] => Toda la coincidencia, $regs[2] => Nombre variable, $regs[3] => relleno de campo, $regs[4] => Alineación
